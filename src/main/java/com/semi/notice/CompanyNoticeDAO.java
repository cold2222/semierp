...
public class CompanyNoticeDAO {
	public static void regContractNotice(int c_type, int c_contract_id) {
		...
		String sql = "insert into CompanyNotice values(seq_companyNotice.nextval, '配車を待つ" + dept + "契約があります。', sysdate, 5, 201, ?)";
		try {
			...
			if(pstmt.executeUpdate() == 1) {
				...
				if (rs.next()) {
					...
					regCheckCompanyNoticeByDeptNo(currval, 201);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void getMainPageNotice(HttpServletRequest request) {
		...
		String sql = "select * from (select * from checkCompanyNotice where ccn_e_no = ? order by ccn_checked)";
		try {
			con = DBManger.connect();
			pstmt = con.prepareStatement(sql);
